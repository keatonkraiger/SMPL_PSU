...
class PartVolume(nn.Module):
    ...
    def close_mesh(self, boundary_vids):
        ...
        self.smplx_mesh.faces = torch.cat([self.smplx_mesh.faces, torch.tensor(new_faces, dtype=torch.long, device=self.device)], dim=0)
    ...
    def extract_part_triangles(self, part_vids, part_fid):
        ...
        if self.part_triangles is None:
            # self.part_triangles = torch.index_select(self.smplx_mesh.vertices, 1, self.smplx_mesh.faces.view(-1)).reshape(batch_size, -1, 3, 3)
